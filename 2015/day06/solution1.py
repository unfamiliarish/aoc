...
def turn_on_lights(
    x_0: int, x_n: int, y_0: int, y_n: int, lights: List[List[bool]]
) -> None:
    ...
    for i in range(x_0, x_n + 1):
        for j in range(y_0, y_n + 1):
            if lights[i][j] is True:
                lights[i][j] = False
            elif lights[i][j] is False:
                lights[i][j] = True
            else:
                raise ValueError(f"invalid value {lights[i][j]} at lights[{i}][{j}]")
...
assert put_up_lights("input_d1_1") == 1000000
assert put_up_lights("input_d1_2") == 1000
assert put_up_lights("input_d1_3") == 0

number_lights = put_up_lights("input")
print(number_lights)
